...
class ForwardKinematics(object):
    ...
    def compute_transforms(self, link_names, joints, joint_values):
        ...
        Tl0=tf.transformations.identity_matrix()
        Tl0_trans=tf.transformations.translation_matrix(joints[0].origin.xyz)
        Tl0_rot=tf.transformations.euler_matrix(joints[0].origin.rpy[0],joints[0].origin.rpy[1],joints[0].origin.rpy[1])
        Tl0=np.dot(Tl0_trans,Tl0_rot)
        
        all_transforms.transforms.append(convert_to_message(Tl0,link_names[0],'world_link'))
        print(joints)
        Tprevious=Tl0
        for idx in range(1,8):
            
            #get the axis of rotation
            axis=joints[idx].axis
            #get the joint value
            joint_value_index=joint_values.name.index(joints[idx].name)
            joint_value=joint_values.position[joint_value_index]
            
            T_rot= tf.transformations.quaternion_matrix(tf.transformations.quaternion_about_axis(joint_value,axis))
            
            T_trans=tf.transformations.translation_matrix(joints[idx].origin.xyz)
            
            #Total transform
            T_local=np.dot(T_trans,T_rot)
            T=np.dot(Tprevious,T_local)
            all_transforms.transforms.append(convert_to_message(T,link_names[idx],'world_link'))
